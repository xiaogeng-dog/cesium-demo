...
export class CesiumCustomPopUp {
  ...
  private init(options: CesiumCustomPopUpOption) {
    ...
    if (
      options.windowCloseCallback &&
      typeof options.windowCloseCallback === 'function'
    ) {
  ...
  public destroy() {
    try {
      if (
        ...
        typeof this.windowCloseCallback === 'function'
      ) {
        this.windowCloseCallback()
      }
      // 移除弹框内容
  ...
  public addPostRender() {
    this.removeListener = this.viewer.scene.postRender.addEventListener(
      this.postRender,
      this,
    )
  }
