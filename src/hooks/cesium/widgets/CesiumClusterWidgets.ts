...
export class CesiumClusterWidgets {
  ...
  constructor(options: ClusterWidgetsOption) {
    ...
    if (
      options.rebuildCluster &&
      typeof options.rebuildCluster === 'function'
    ) {
      this.rebuildCluster = options.rebuildCluster
    }
    if (
      options.selectedEntity &&
      typeof options.selectedEntity === 'function'
    ) {
  ...
  private async initCluser(options: ClusterWidgetsOption) {
    const geoJsonDataSource: GeoJsonDataSource = await GeoJsonDataSource.load(
      this.data,
    )
    // 设置聚合的形态
    geoJsonDataSource.clustering.enabled =
      typeof options.clustering?.enabled === 'boolean'
        ? options.clustering?.enabled
        : true
    geoJsonDataSource.clustering.clusterLabels =
      typeof options.clustering?.clusterLabels === 'boolean'
        ? options.clustering?.clusterLabels
        : false
    geoJsonDataSource.clustering.pixelRange =
      options.clustering?.pixelRange || 15
    geoJsonDataSource.clustering.minimumClusterSize =
      options.clustering?.minimumClusterSize || 3
    ...
    this.cluserLayerResource = await this.viewer.dataSources.add(
