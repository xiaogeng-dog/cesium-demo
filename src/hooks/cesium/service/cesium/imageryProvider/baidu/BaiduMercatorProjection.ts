 ...
 * 百度地图
 */
import type { MapPoint } from '../../../../types/CommonType'
...
class BaiduMercatorProjection {
  ...
  getDistanceByMC(point1: MapPoint, point2: MapPoint) {
    ...
    const x1: number = this.toRadians(point1['lng'])
    const y1: number = this.toRadians(point1['lat'])
    point2 = this.convertMC2LL(point2)
    if (!point2) {
      return 0
    }
    const x2: number = this.toRadians(point2['lng'])
    const y2: number = this.toRadians(point2['lat'])
  ...
  convertLL2MC(point: MapPoint): MapPoint {
    ...
    if (this.isWgs84) {
      const mercator: Record<any, any> = {}
      const earthRad: number = 6378137.0
      mercator.lng = ((point.lng * Math.PI) / 180) * earthRad
      const a: number = (point.lat * Math.PI) / 180
    ...
    const temp: MapPoint = { lng: point['lng'], lat: point['lat'] }
    let factor: any = undefined
    ...
    const mc: MapPoint = this.convertor(point, factor)
    return {
  ...
  convertor(fromPoint: MapPoint, factor: number[]): MapPoint {
    ...
    const temp: number = Math.abs(fromPoint['lat']) / factor[9]
    let y: number =
      factor[2] +
      factor[3] * temp +
      factor[4] * temp * temp +
  ...
  lngLatToPoint<T>(point: any): Record<string, any> {
    const mercator: MapPoint = this.convertLL2MC(point)
    return {
      x: mercator['lng'],
      y: mercator['lat']
    }
  ...
  pointToLngLat<T>(point: Record<any, any>): MapPoint {
    const mercator: MapPoint = { lng: point.x, lat: point.y }
  ...
  pointToPixel(point: MapPoint, zoom: number, mapCenter: MapPoint, mapSize: Record<any, any>) {
    ...
    const zoomUnits: number = this.getZoomUnits(zoom)
    const x: number = Math.round((point['lng'] - mapCenter['lng']) / zoomUnits + mapSize.width / 2)
    const y: number = Math.round((mapCenter['lat'] - point['lat']) / zoomUnits + mapSize.height / 2)
    return { x, y }
  }
  ...
  pixelToPoint(
    ...
    zoom: number,
    mapCenter: MapPoint,
    mapSize: Record<any, any>
  ) {
    if (!pixel) {
      return
    }
    const zoomUnits: number = this.getZoomUnits(zoom)
    const lng: number = mapCenter['lng'] + zoomUnits * (pixel.x - mapSize.width / 2)
    const lat: number = mapCenter['lat'] - zoomUnits * (pixel.y - mapSize.height / 2)
