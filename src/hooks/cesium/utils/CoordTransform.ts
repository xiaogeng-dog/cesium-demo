...
class CoordTransform {
  ...
  static BD09ToGCJ02(lng: number, lat: number): number[] {
    ...
    const z: number = Math.sqrt(x * x + y * y) - 0.00002 * Math.sin(y * BD_FACTOR)
    const theta: number = Math.atan2(y, x) - 0.000003 * Math.cos(x * BD_FACTOR)
  ...
  static GCJ02ToBD09(lng: number, lat: number): number[] {
    ...
    const theta: number = Math.atan2(lat, lng) + 0.000003 * Math.cos(lng * BD_FACTOR)
  ...
  static GCJ02ToWGS84(lng: number, lat: number): number[] {
    ...
    if (this.out_of_china(lng, lat)) {
      ...
      const d: number[] = this.delta(lng, lat)
      const mgLng = lng + d[0]
      const mgLat = lat + d[1]
